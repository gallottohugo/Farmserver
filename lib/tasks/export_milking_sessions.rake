...
desc "Export data for uploading to tambero.com"
  task :export_milking_sessions => :environment do
  ...
  if @last_pedometry_date
    ...
    MilkingSession.all(:conditions=> ['status not in (?)',[100,21030,21040,500]]).each do |milkingsession|  
      ...
      if @animal_milking.count > 0 or @pedometry.count  > 0
        ...
        url = api.tambero_url+"/api/export?userid="+api.tambero_user_id+"&apikey="+api.tambero_api_key+"&apiobjecttype=milkings&apilicense=rodegserver&apilang="+api.language+"&apilist=equal_date&recordtype=new&recorddate="+@date.to_s
        puts url.to_s
        cuenta = 0
        begin
        ...
        if data == []
          ...
          url_new_milking = api.tambero_url+"/api/import?userid="+api.tambero_user_id+"&apikey="+api.tambero_api_key+"&apiobjecttype=milkings&apilicense=rodegserver&apilang="+api.language
          ...
          puts response1
            ...
            new_url = api.tambero_url+"/api/export?userid="+api.tambero_user_id+"&apikey="+api.tambero_api_key+"&apiobjecttype=milkings&apilicense=rodegserver&apilang="+api.language+"&apilist=equal_date&recordtype=new&recorddate="+@date.to_s
            ...
            url_new_detail = api.tambero_url+"/api/import?userid="+api.tambero_user_id+"&apikey="+api.tambero_api_key+"&apiobjecttype=milkingdetails&apilicense=rodegserver&apilang="+api.language+"&id_animal_control="+@id_animal_control.to_s
          ...
          data.each do |d|
            if @animal_control = d["animal_control"]
              ...
              url_update_detail = api.tambero_url+"/api/update?userid="+api.tambero_user_id+"&apikey="+api.tambero_api_key+"&apiobjecttype=milkingdetails&apilicense=rodegserver&apilang="+api.language+"&id_animal_control="+@id_animal_control.to_s
  ...
  task :reset_export => :environment do
  ...
  def dns_tambero
    ...
    @url_dns = @api.tambero_url+'/api/dns?userid='+@api.tambero_user_id+'&apikey='+@api.tambero_api_key+'&apilicense=rodegserver&apiip='+@mi_ip1+'&apiport=1111&apilang='+@api.language
...
def update_alarms
      if @api = TamberoApi.first
        url_alarm = @api.tambero_url+'/api/alarms?userid='+@api.tambero_user_id+'&apikey='+@api.tambero_api_key+'&apilicense=rodegserver&apilang='+@api.language
